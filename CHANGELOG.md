...
「钱说不清」那一层最便宜的两个补丁，现有三家当天用上，谁都不依赖。

一、老板事后问「#12 那单的付款截图呢」，以前答不出来：截图存在盘上，media_id 只挂在
「付款核验」那条 pending 上，一核准 pending 就被删了，写进台账的订单记录不带图。
现在凭证单独进 `d["proofs"]`，**只增不删**，按订单号挂，核准/驳回只改状态不删记录。

二、买家下了单不付款，没人跟，老板也不知道有这么一单悬着。现在下单超过 2 小时
（`FOLLOWUP_AFTER_MIN`）、名下没有任何付款凭证、且没催过的单，系统替老板问买家一句，
并给老板推一条「我已经替您问了」。**只催一次**，不做审批链、不做多次催付、不做自动关单。

#### Added
- `dianxiaoli_core.py` — 凭证台账 `d["proofs"]`：
  `{id（= pending 单号）, order_id（关联订单号，认不准就留空）, userid, name, media, amount, ts, kind, status}`，
  `status` ∈ 待核 / 已核准 / 已驳回。`add_pending()` 新增可选参数 `order_id=""`，
  `kind` 是付款核验 / 金额异常且带图时自动补一条台账。
  `proofs_for(d, order_id)` 取台账、`proof_text(d, order_id)` 出老板看的那几行。
- `dianxiaoli_core.py` 老板指令 —「12号凭证 / #12凭证 / 12号的凭证 / 凭证12」四种写法都认，
  每条凭证一行 `#<id> {状态} {金额} {时间}`，金额没读出来的写「金额待核」而不是摆一个 `¥0`。
  有图时末尾附一句「图在控制台「付款凭证」那一栏，点开看大图。」——**对话里不给看图链接**：
  相对路径老板在微信里点不开，带上钥匙又等于把 BOSS_KEY 塞进对话，两头都不行。
  查不到时：「单 #N 还没收到付款凭证。买家发了截图我会马上记上。」
  只说「凭证」没带单号时教他怎么问：「说个单号就行，比如「12号凭证」。最近几张在控制台「付款凭证」那一栏。」
- `dianxiaoli_core.py` — `GET /boss/proofs?key=&order=N`：`order` 省略就返回全部（最近 50 条），
  无 key 401。老板端页面在「今日日报」上方新增「🧾 付款凭证」区，列最近 10 条
  （买家名 / 状态 / 金额 / 订单号 / 缩略图，点击复用现有 `zoom()` 放大），
  数据来自 `/boss/state` 新增的 `proofs` 字段（最近 10 条、最新在前）。
- `followup.py` — 跟单催付。`check(core=None, now=None)` 扫 `d["pending"]` 里的「订单核准」，
  **三个条件全满足**才催：① 下单距今 ≥ `FOLLOWUP_AFTER_MIN` 分钟；② 该买家名下没有付款核验 /
  金额异常的待办，且台账里没有指向这个单号的凭证；③ 这条没有 `followed_up` 字段。
  **先把 `followed_up` 落盘、再发消息**：发消息要走企微、可能几秒，拿着几秒前的旧快照回头 `save`
  会把这期间买家那次写入（订单、库存、会话记忆）整个覆盖掉。落盘前重新 `load` 并复查一次
  `followed_up`，所以并发下也只催一次。
  台账时间戳只有月日，按「当前年份 + 该月日」解析，算出来比现在晚超过 1 天就当去年（12-31 遇上 01-02），
  解析不了的直接跳过这条、不抛。
  开关 `FOLLOWUP_ENABLED`（默认 `1`，设 `0` 关掉）、间隔 `FOLLOWUP_AFTER_MIN`（默认 `120`）。
  文案过红线：不判断钱到没到、不逼单——买家那句是「我先给您留着…暂时不方便也说一句，我帮您留到明天」，
  没有「请尽快付款」这种话；台账里叫「顾客」的生客不会被念成「顾客您好」，直接「您好，您订的…」，
  有真称呼时才「张三您好」。老板那句是「下单 N 小时没下文，我已经替您问了一句」，
  **不满一小时按分钟说**（`span_cn()`：35 分钟就是「35 分钟」，不许四舍五入成「1 小时」）。
- `dianxiaoli_core.py` — `/status` 新增 `followup` 段：`enabled` / `after_min` / `pending_watched`
  （当前「订单核准」且未催过的条数）。
- `testing/test_proof_followup.py` — 66 项：金额一致 / 金额异常两条路径的 `order_id`、
  第三分支「只有一笔待核准订单」能关联而「两笔」留空、核准与驳回各自改对状态、
  pending 删掉后台账仍在、四种问法、金额待核不显示 ¥0、只说「凭证」不掉进买家兜底、
  查不到的文案、**回话里不含真实 BOSS_KEY**（把 key 设成特征串再断言）、不给点不开的 `/boss/media` 链接、
  `/boss/proofs` 的 401 / 带 order 过滤 / 不带 order 全量、`/boss/state.proofs`、页面区块标记；
  不到时间不催、到点催一次并打标记、第二次不再催、已发凭证的不催（含凭证已核准、pending 里没了的情况）、
  `FOLLOWUP_ENABLED=0` 不催、买家文案不含「请尽快付款 / 尽快 / 到账 / 收到钱 / 催」且带商品描述、
  生客不被叫「顾客您好」、老板文案带单号、`span_cn()` 的分钟 / 小时口径与 35 分钟实跑、
  **催付发消息期间别人的写入不被旧快照覆盖**（在 `_notify` 里插一次并发写盘再断言它还在）、
  跨年 12-31 不被误判成未来、垃圾时间戳不抛、`/status.followup`。

#### Changed
- `dianxiaoli_core.py` `_image_payment_flow()` — 金额一致与金额异常两个分支把 `order_id=latest["id"]`
  传下去；读不出金额的第三分支只在该买家名下**恰好一笔**待核准订单时才认那一笔，
  两笔以上有歧义就留空——**不猜**。`_do_act()` 处理付款核验 / 金额异常时同步把台账里
  `id == pid` 那条的状态改成已核准 / 已驳回（pending 照旧删掉，台账那条留着）。
  `add_pending()` 写进 pending 的条目也多了 `order_id` 字段。
- `agent_c_audit.py` `_loop()` — 审计循环里顺带每 5 分钟跑一次 `followup.check()`，
  不另起线程；整段 try/except 吞异常并 `core.record_error`，绝不影响审计调度。
  `AUDIT_SCHEDULER=0` 时 loop 本来就不启动，催付跟着一起关掉。

#### 回归
- **12 套测试全绿 · 计数口径 428 项**：中文门禁 28 + 工厂门禁 26 + audit 44 + catalog_import 24 +
  llm_brain 23 + notify 17 + vision 41 + wecom_core 12 + demo_catalog 42 + boss_notify 99 +
  cold_start 6 + proof_followup 66（新增）。另加 `dianxiaoli_skills/tests/test_skill_loader.py`，
  它只报 OK / 不报条数，所以不计入 415。已有测试断言一条没改。
